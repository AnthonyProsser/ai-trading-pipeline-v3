...
**Idea-search loop** (2026-07-07, cross-phase, unattended multi-day GPU run). Started to find a directionally-profitable recipe — every prior benchmark run showed DA ≈ coin-flip. Each idea gets its own `idea-*` branch off `main`. The mandatory per-change subagent review triggers are suspended for this loop (see ECC integrations note below); re-invoke the auditor trio by hand once an idea is chosen for refinement. `experiments/idea_search/ledger.jsonl` is the durable record of every idea (append-only, one JSON per idea) and is committed **directly to `main`** — exempt from the one-branch-per-idea rule, since it is loop bookkeeping, not idea code. **Mode change (2026-07-08, user-directed): FULL 78-fold runs only — no more capped bake-offs** (capped screens burned the session-token budget and were misleading), **Sonnet-only subagents** (never Opus), minimal orchestrator tokens, keep launching the next idea unattended after each run finishes. New tool `scripts/run_full_training.py` (branch `tooling-full-run`, cherry-picked onto idea branches): headless `train_all_folds` driver → auto-promotes to `checkpoints/finished/` (benchmark-app visible). **Results so far:** capped fold-77 screens rejected ideas 1 (clock features), 2 (multi-scale returns), 3 (lookback 720), 4 (aux direction head — degraded calibration, informative) at DA ~0.48; a fold-25 diagnostic then showed fold 77 is a near-WORST fold, so those rejections were pessimistic. Full 78-fold runs: **baseline mean DA 0.5155 (63/78 folds >0.5), idea-2 multi-scale 0.5201 (69/78)** — a real small directional edge exists at full training that capped screens hid; likely still sub-fee (benchmark app shows PnL). Caveat: benchmarking a run's checkpoints requires the matching branch checked out (feature count/architecture must match the loaded code). User's parked feature backlog (Krafer video, Tier 1–3) + open questions for the user: ledger `backlog-video` entry and `experiments/idea_search/QUESTIONS.md`.
