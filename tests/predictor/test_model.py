...
def test_forward_maps_seven_input_features_to_five_output_dims() -> None:
    # idea-05-swinglevels: NUM_INPUT_FEATURES (7: 5 OHLCV + 2 swing-high/low distance
    # features) is decoupled from NUM_OUTPUT_DIMS (5: OHLCV only) -- the model still
    # predicts only OHLCV; RevIN and the patch embedding still cover all 7 inputs.
    torch = pytest.importorskip("torch")
    from src.predictor.model import PatchTST
