...
## 2026-07-07 — idea-04: auxiliary directional BCE head (shared encoder)
- loss: added a fourth, optional term to `predictor_loss` — `AUX_DIRECTION_WEIGHT × BCE(direction_logit, 1{target_close_cum_H > 0})`. `direction_logit` comes from a new training-only `PatchTST.direction_head` (a `Linear` sharing the encoder trunk via `forward_with_direction`, which also returns the identical quantile tensor `forward` produces). `predictor_loss` gained keyword-only `direction_logit: Tensor | None = None` / `aux_weight: float = PREDICTOR.AUX_DIRECTION_WEIGHT`; `LossComponents` gained an `aux` field (before `total`). Omitting `direction_logit` (every pre-existing caller) yields `aux == 0` and an unchanged `total` — backward compatible.
- New `constants.py` value: `PredictorConfig.AUX_DIRECTION_WEIGHT = 0.5`.
- `src/predictor/model.py::PatchTST`: refactored `forward` into a shared `_trunk`/`_quantiles` split (no behavior change — `forward(x)`'s signature and return are unchanged, still only the quantile tensor). `forward_with_direction` is training-only and dropped at inference (deploy/benchmark call `forward`, unaffected).
- `src/predictor/training.py`: both the training step and `_evaluate` (fold validation) now call `model.forward_with_direction` and pass `direction_logit` into `predictor_loss`, so the aux term is included in `val_total` (early stopping selects on it). SSE `batch`/`val` log payloads gained `aux`/`train_aux`/`val_aux` fields alongside the existing pinball/direction/coverage siblings (additive — no existing field removed or renamed). `_evaluate`'s return tuple grew from 4 to 5 floats (`..., aux, total`); `_evaluate`/`train_one_fold`'s `model` param narrowed from `torch.nn.Module` to `PatchTST` (mypy — `forward_with_direction` isn't a base `nn.Module` method).
- Reason: directional accuracy has been stuck at ~coin-flip (~0.48–0.53) across every prior idea (features, horizon, lookback). Hypothesis: a direct directional gradient into the shared encoder trunk enriches its representation so the quantile head's q50 sign becomes more accurate. Unvalidated pending the idea-04 capped bake-off (fast screen before any full run, per the idea-search loop protocol).
- Source: idea-search loop, idea-04 proposal (branch `idea-04-auxdirhead`).
...
## 2026-07-05 — Benchmark: completeness-gated scoring + per-run "Benchmark all"
- Amended `benchmark_run_grouping`: a run is now SCORED and ranked only when it is **complete** — every one of its *compatible* fold-checkpoints has a benchmark result. Previously a run appeared (scored) as soon as ≥1 fold was benchmarked, and a run with 0 benchmarked folds was dropped entirely. Now: complete runs rank at the top by `profitable_fraction` (unchanged); **incomplete** runs (partial OR zero benchmarked, including a freshly-finished run) appear at the **bottom** with **no score** (score fields null), ordered fewest-`n_pending` first (tie: `git_sha`). A run with **no compatible fold at all** (nothing benchmarkable) is still dropped. New per-run fields on `/api/leaderboard`: `complete` (bool), `n_compatible`, `n_pending`.
- New decision key `benchmark_batch_benchmark`: a per-run "Benchmark all" enqueues every remaining *compatible, not-yet-benchmarked* fold of one run, back-to-back on the existing one-job-at-a-time runner. New endpoint `POST /api/benchmark/run/start` (body `{git_sha, constants_sha8}`; server derives the pending stems — the client never supplies a stem list). `BenchmarkRunner` gained `start_batch(list[str])` (single-stem `start` now delegates to it); each stem is best-effort (one failing fold alerts but does not abort the batch). No new `constants.py` values.
- Reason: user asked that only fully-benchmarked runs be scored, and that "Benchmark all" be reachable for the unscored runs at the bottom — the case (a run fresh out of training with 0 benchmarked folds) where batch-benchmarking is most useful and was previously unreachable because such runs were hidden.
- Flagged unspecced (sub-details mine): "complete" is measured over *compatible* folds only (incompatible folds can never be benchmarked, so they don't block completion); incomplete-run ordering = fewest pending first; the endpoint derives stems server-side rather than trusting the client.
- Source: conversation (user request + clarification).
...
- Source: merge conflict resolution, 2026-07-04.
...
- `train_all_folds` gained `finished_dir` param; on natural full-run completion it copies each fold's gate-evaluated checkpoint there and emits terminal `state:"done"` (+`promoted` count) instead of `"stopped"` (see 2026-07-04 merge-reconciliation entry above — originally emitted as `state:"completed"` before merging with `phase-15-controls-fix`); `fold_complete` payload gained `stem`. `FoldRecord` gained `stem: NotRequired[str]`. Benchmark `create_runner` reads `FINISHED_DIR`; added `GET /api/analysis/{stem}` (benchmark⋈training join) and `BenchmarkRunner.training_metrics_dir`.
