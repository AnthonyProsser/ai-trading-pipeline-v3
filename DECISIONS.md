...
- **predictor_checkpoint_save_format**: weights = wrapped dict `{"state_dict" (CPU tensors), "lookback", "constants_sha256", "trained_through_ts_utc", "train_q90_coverage"}` via `torch.save`, loadable under `weights_only=True`; scaler = `pickle`. `deploy_predictor.py` reads both (state_dict via the `"state_dict"` key, scaler via `pickle.load`). `trained_through_ts_utc` is ISO8601 UTC (e.g. `2020-01-05T00:00:00Z`) = last TRAIN candle. Caveat: pickle couples the scaler artifact to the Python version and the `PerFoldMinMaxScaler` class definition — a refactor of either silently breaks `pickle.load`; the SHA256 manifest does not protect against this. Re-train/re-save scalers after any such change.
